...
import com.parqduetUtils.{SparkSsUtils, appUtils}
...
object AppTags {
  def main(args: Array[String]): Unit = {
    ...
    val value = df.filter(tagUtils.userIDone).map(row=>{
      //唯一用户标识
      val userOneID = tagUtils.getOneUserID(row)
      //广告标签
      ...
      (userOneID,adList++appList++areaList++devList++keyList++busines)
    }).rdd.reduceByKey((list1,list2)=>{
      list1:::list2
    }).map(x=>{(x._1,x._2.groupBy(_._1).map(x=>(x._1,x._2.size)).toList)})
      .map{
        //偏函数
        case (userid,tags)=>{
          val put = new Put(Bytes.toBytes(userid))
          val tag: String = tags.map(x=>(x._1+":"+x._2)).mkString(",")
          // 对应的rowKey下面的列  列名 列的value值
          put.addImmutable(Bytes.toBytes("tags"),Bytes.toBytes("2019-07-26"),Bytes.toBytes(tag))
